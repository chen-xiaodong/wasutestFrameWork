    ...
    def __call__(self, func):
        ...
        def log_wrapper(*args, **kwargs):
            ...
            try:
                ...
                self.logger.info("func args:" + args1 + "\t" + args2)
                self.logger.info("func: " + func.__name__ + "。 result:  " + str(result))
                return result
            except NoSuchElementException as e1:
                self.logger.error("没有找到元素异常。执行方法 %s,参数：%s %s", func.__name__, args1, args2)
                # retry_func(self.retry_time, func, *args, **kwargs)
                # self.retry_time = self.retry_time - 1
            except FileNotFoundError as e2:
                self.logger.error("文件未找到。执行方法 %s,参数：%s %s", func.__name__, args1, args2, exc_info=True)
            except AttributeError as e3:
                self.logger.error("属性异常。没有该属性。执行方法 %s,参数：%s %s", func.__name__, args1, args2, exc_info=True)
